...
async function updateOrderStatus(
  ...
  const { data: order, error: fetchError } = await supabase
    .from("orders")
    .select("id, status, room_id, items")
  ...
  // Extend room time based on purchased minutes
  try {
    const items = order.items as any[];
    const timeExtension = items.find((item: any) => item.name.includes('minutes'));
    if (timeExtension) {
      const minutes = parseInt(timeExtension.name.match(/\d+/)?.[0] || '0');
      if (minutes > 0) {
    ...
    console.error("Failed to extend room time:", err);
  }
